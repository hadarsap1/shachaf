...
//
// The one true restriction is members-only: an event opened inside a group or
// committee for its members shows only to CURRENT members, with no admin
// bypass, so it disappears the moment someone leaves.
export function isEventVisibleTo(ev, { entityIds = [] } = {}) {
  if (!ev) return false
