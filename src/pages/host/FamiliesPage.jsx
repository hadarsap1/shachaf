    ...
    <>
      ...
      <div className="fixed top-0 right-0 h-full w-full max-w-sm bg-white shadow-2xl z-50 flex flex-col animate-slide-from-right" dir="rtl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          ...
          <h2 className="font-bold text-gray-800">פרטי משפחה</h2>
        </div>
        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5">
          <div className="flex flex-col items-center gap-3">
            <div className="avatar w-16 h-16 text-2xl bg-primary-100 text-primary-700">
              {family.name?.[0] || '?'}
            </div>
            <div className="text-center">
              <div className="font-bold text-gray-800 text-base">{family.name}</div>
              <span className="text-xs bg-primary-50 text-primary-700 border border-primary-200 px-2 py-0.5 rounded-full font-medium">משפחה חדשה</span>
            </div>
          </div>
...
function FamilyCard({ family, taskCounts, onClick }) {
  ...
  return (
    <div className="card p-5 cursor-pointer hover:bg-gray-50 transition-colors" onClick={onClick}>
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          {isUrl(family.avatar)
            ? <img src={family.avatar} alt="" className="w-12 h-12 rounded-full object-cover flex-shrink-0" />
            : <div className="avatar w-12 h-12 text-lg bg-primary-100 text-primary-700">{family.name?.[0] || '?'}</div>
          }
      ...
      <div className="mb-4">
        <div className="flex justify-between text-xs text-gray-500 mb-1.5">
          ...
          <span className="font-medium text-gray-700">{done}/{total}</span>
        </div>
